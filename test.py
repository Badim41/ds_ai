...
class GoogleSpeechToText:
    def __init__(self, endpoint, lang, recognition_model, api_credentials=None):
        credentials = google.oauth2.service_account.Credentials.from_service_account_file(
            api_credentials) if api_credentials else grpc.local_channel_credentials()
        LocalSpeechGrpcTransport = type('LocalSpeechGrpcTransport', (
        google.cloud.speech_v1.gapic.transports.speech_grpc_transport.SpeechGrpcTransport,), dict(
            create_channel=lambda self, address, credentials, **kwargs: grpc.secure_channel(address, credentials,
                                                                                            **kwargs)))
        client_options = dict(api_endpoint=endpoint)

        self.client = google.cloud.speech_v1.SpeechClient(credentials=credentials,
                                                          client_options=client_options) if api_credentials else google.cloud.speech_v1.SpeechClient(
            transport=LocalSpeechGrpcTransport(address=endpoint, credentials=credentials),
    ...
    def transcribe(self, pcm_s16le, sample_rate, num_channels):
        res = self.client.recognize(
            dict(audio_channel_count=num_channels, encoding='LINEAR16', sample_rate_hertz=sample_rate,
                 language_code=self.lang, model=self.recognition_model), dict(content=pcm_s16le))
...
class DebugDumpRawAudio:
    ...
    def transcribe(self, pcm_s16le, sample_rate, num_channels):
        audio_path = os.path.join(self.debug_dir,
                                  f'{int(time.time())}.{random.randint(1000, 9999)}._s16le_hz{sample_rate}_ch{num_channels}.wav')
...
class BufferAudioSink(discord.AudioSink):
    ...
    def write(self, voice_data):
        ...
        need_flush = (self.buffer_pointer >= self.BUFFER_FRAME_COUNT - 2) or (
                    not speaking and self.buffer_pointer > 0.5 * self.BUFFER_FRAME_COUNT)  # or (self.speaker is not None and speaker != self.speaker)

        if speaking:
            self.buffer[
            (self.buffer_pointer * self.NUM_SAMPLES): ((1 + self.buffer_pointer) * self.NUM_SAMPLES)] = frame
...
class DiscordBotClient(discord.Client):
    def __init__(self, transcriber, text_channel_id=None, voice_channel_id=None, text_channel_name=None,
                 voice_channel_name=None, *args, **kwargs):
    ...
    async def message_sending_loop(self, timeout=0.25):
        while True:
            ...
            for speaker, hyp in self.messages:
                ...
                speaker_pretty = str(
                    member.nick if member.nick is not None else member) if member is not None else 'N/A'
    ...
    async def on_ready(self):
        ...
        self.text_channel = discord.utils.get(self.get_all_channels(), **(
            dict(id=self.text_channel_id) if self.text_channel_id is not None else dict(name=self.text_channel_name)))
        self.voice_channel = discord.utils.get(self.get_all_channels(), **(
            dict(id=self.voice_channel_id) if self.voice_channel_id is not None else dict(
...
if __name__ == '__main__':
    ...
    parser.add_argument('--discord-bot-token', required=True)
    ...
    parser.add_argument('--recognition-model', default='phone_call',
                        choices=['phone_call', 'default', 'video', 'command_and_search'])
    ...
    transcriber = GoogleSpeechToText(endpoint=args.endpoint, recognition_model=args.recognition_model, lang=args.lang,
                                     api_credentials=args.google_api_credentials_file) if not args.debug else DebugDumpRawAudio(
        args.debug)
    bot = DiscordBotClient(text_channel_id=args.text_channel_id, voice_channel_id=args.voice_channel_id,
                           text_channel_name=args.text_channel_name, voice_channel_name=args.voice_channel_name,
