...
async def __generate_video(ctx,
                           ...
                           seed: Option(int, description='Сид генератора (random)', required=False, default=None,
                                        min_value=1, max_value=9999999999),
        ...
        while not_speak < 120:
            ...
            if spoken_text:
                ...
                prompt = (
                    ...
                    f"## 3. ОБЯЗАТЕЛЬНО дай ответ на предложение: \"{last_line.split(':')[1:]}\"\n"
                    f"### Предложение пользователей войс чата: \"{spoken_text}\"\n"
                    f"## 4. В диалоге должно быть ДО 3 фраз, НЕ БОЛЬШЕ\n"
                ...
                theme_user = await self.run_gpt(f"Придумай новую тему для этого диалога:\n"
                                                f"# Пользователь:\n {spoken_text}\n"
                                                f"# Диалог:\n{result}\n"
                                                f"В ответе выведи 3-6 слова в качестве следующей темы для диалога.\n"
