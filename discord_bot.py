...
class Dialog_AI:
    ...
    async def gpt_dialog(self):
        prompt = (
            ...
            f"#Информация\n{'.'.join(self.infos)}.\n#{self.global_prompt}. \n\n"
            f"#Требования\n1.Персонажи должны соответствовать своему образу насколько это возможно.\n"
            f"2.Обязательно в конце диалога напиши кратко, что было в этом диалоге и что должно произойти в следующем диалоге.\n"
            f"3.Выведи диалог в таком формате:\n[Говорящий]: [текст, который он произносит]")
        ...
        while self.alive:
            try:
                ...
                prompt = (
                    f"#Задача\nВы собираетесь продолжить диалог между {', '.join(self.names)}.\n #Тема диалога\n {theme_temp}.\n"
                    f"#Информация\n{'.'.join(self.infos)}.\n#{self.global_prompt}. \n\n"
                    f"#Требования\n1.Персонажи должны соответствовать своему образу насколько это возможно.\n"
                    f"2.Не пиши приветствие в начале диалога.\n"
                    f"3.Не повторяй прошлые диалоги! Прошлый диалоги закончился на:\"{dialog_next}\". {spoken_text}"
                    f"4.Обязательно в конце диалога напиши кратко, что было в этом диалоге и что должно произойти в следующем диалоге.\n"
                    f"5.Выведи диалог в таком формате:\n[Говорящий]: [текст, который он произносит]")
